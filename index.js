...
function customAssign(...objects) {
  ...
  let objectNew = Object.assign(...objects);
  for (const item in objectNew) {
    if (typeof objectNew[item] === "number") {
      objectNew[item] = String(objectNew[item]);
    }
  }
...
function customObjectFromEntries(entries) {
  ...
  const entriesNew = entries.map(([key, value]) => [key.toString(), value]);
  // return entriesNew;
  const obj = Object.fromEntries(entriesNew);
  return obj;
...
function filterObjectsByKey(arr, key) {
  ...
  return arr.filter((el) => el.hasOwnProperty(key) === true);
...
function checkOwnership(obj1, obj2, key) {
  if (obj1.hasOwnProperty(key) && obj2.hasOwnProperty(key) === true) {
